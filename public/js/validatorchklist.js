...
$(document).ready(function () {
	//history.forward(1);
    $('input[type=text],input[type=password],textarea').not('.ckeditor,.amhfld').on('keyup', function () {
        return blockspecialchar_first(this);
    });
    $('input[type=text],textarea').not('.amhfld').on('blur', function () {
    ...
    //FUnction to disable speacial characters on keyup :by: T Ketaki Debadarshini 
     $('input[type=text],textarea').not('.amhfld').on('keypress', function (e) {
...
//            
            ...
            if (!XRegExp('\\pL+$').test($(this).val()) || forbiddenChars.test($(this).val())) {
...
//            console.log(XRegExp('\\p{Ethiopic}').test(str))
            
            if ( !XRegExp('\\p{Ethiopic}').test(str) ) {
                console.log('sql script detected')
...
function minLength(controlId, ctrlLen, fieldName)
{
    if ($('#' + controlId).val().length < ctrlLen && $('#' + controlId).val().length > 0)
    {
        viewAlert(fieldName + ' cannot less than ' + ctrlLen + ' characters !!!',controlId);
        $('#' + controlId).focus();
        return false;
    }
    return true;
